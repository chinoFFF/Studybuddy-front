// components/GenerateFlashcardsModal.tsx
import React, { useState } from 'react';
import { flashcardsApi } from '../api/flashcards.api';
import type { FlashcardDeckWithCards } from '../types/flashcards';
...
interface GenerateFlashcardsModalProps {
  ...
  onGenerated?: (deck: FlashcardDeckWithCards) => void;
}
...
export const GenerateFlashcardsModal: React.FC<GenerateFlashcardsModalProps> = ({
  ...
  const [generatedDeck, setGeneratedDeck] = useState<FlashcardDeckWithCards | null>(null);
  ...
  const handleGenerate = async (e: React.FormEvent) => {
    ...
    try {
      const deck = await flashcardsApi.generateDeck({
      ...
      console.error(err);
      setError('No se pudo generar el mazo de flashcards. Intenta de nuevo.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <>
      <button
        ...
        className="w-full py-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 font-bold text-xs rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
      >
        📝 Generar Flashcards
      </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4 transition-opacity duration-300">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden border border-gray-100">
            ...
            <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
              <h3 className="text-xl font-bold text-gray-800">Generar Flashcards</h3>
            ...
            <div className="p-6">
              {!generatedDeck ? (
                <form onSubmit={handleGenerate} className="space-y-4">
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-2">Número de tarjetas</label>
                    <input
                      type="number"
                      min={1}
                      max={50}
                  ...
                  <div>
                    ...
                    <textarea
                      rows={3}
                      placeholder="Ej. Enfócate en las definiciones clave del capítulo 2"
                      value={description}
                  ...
                  <button
                    type="submit"
                    disabled={loading}
                    className="w-full py-3 bg-indigo-600 hover:bg-indigo-700 disabled:bg-gray-300 text-white font-semibold rounded-xl transition-colors shadow-sm"
                  >
                ...
                <div className="space-y-4 text-center">
                  ...
                  <div>
                    <h4 className="font-bold text-gray-800 text-lg">¡Mazo "{generatedDeck.title}" listo!</h4>
                    <p className="text-sm text-gray-500 mt-1">
                      Se generaron {generatedDeck.flashcards.length} tarjetas.
                    </p>
                  </div>
                  <button
                    onClick={handleClose}
                    className="mt-2 w-full py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold rounded-xl transition-colors shadow-sm"
                  >
                    Listo
                  </button>
                </div>
